...
from src.models import MapperModel
...
def mapping_model(lm_hidden_state):
    """
    Loads the trained language model and runs it on lm output embeddings
    """
    model_dict = torch.load("model.pth")
    model = MapperModel()
    model.load_state_dict(model_dict)

    output = model(lm_hidden_state)
    return output
...
def text_to_image(text, mapping_model,
    ...
    with torch.no_grad():
        ...
        #
        lm_hidden_states_first_example = rearrange(lm_hidden_states_first_example, 'a b -> 1 a b')
        print(lm_hidden_states_first_example.shape)
...
# Testing our model by passing a sentence and getting a generated image
